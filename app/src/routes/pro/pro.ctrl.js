...
const process = {
    ...
    postApply: (req, res) => {
        const postID = parseInt(req.params.postID);
        const nickname = req.session.nickname;
        const query = "SELECT ProfessionalEdu.postID, ProfessionalEdu.title, ProfessionalEdu.category, ProfessionalEdu.status, ProfessionalEdu.price, users.id, users.name, users.nickname, users.mphone FROM ProfessionalEdu INNER JOIN users ON users.nickname = ? WHERE postID = ?;";
        const applydate = new Date();
        db.query(query, [nickname, postID], (err, result) => {
            if (err) console.log(err);
            if (result) {
                const query2 = "INSERT INTO proapply (postID, title, category, recruitStatus, price, id, name, nickname, mphone, applydate, applyStatus) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'wait');";
                dbdata = [
                    postID,
                    result[0].title,
                    result[0].category,
