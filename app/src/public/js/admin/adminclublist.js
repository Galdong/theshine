...
function search() {
    ...
    fetch(`/admin/clublist/search?keyword=${input}&filter=${filter}`)
...
function renderSearchResult(data) {
    const table = document.querySelector('#clublist');
    ...
    if (data.length > 0) {
        data.forEach(item => {
            ...
            row.innerHTML = `
                <td class="list-title"><a href="/admin/clubview/${item.postID}">${item.title}</a></td>
    ...
    allBtn.innerHTML = '<a href="/admin/clublist">전체보기</a>';
...
function writePost() {
    location.href = "/admin/club/write";
}
    ...
    function renderPage(currentPage, totalPages) {
        ...
        if (currentGroup > 1) {
            ...
            prevGroupLi.addEventListener('click', () => {
                window.location.href = `/admin/clublist?page=${startPage - pagesPerGroup}`;
            });
        ...
        for (let i = startPage; i <= endPage; i++) {
            ...
            li.addEventListener('click', () => {
                window.location.href = `/admin/clublist?page=${i}`;
            });
